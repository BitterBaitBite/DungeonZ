...
class MapLayer final : public sf::Drawable {
    ...
    private:
        ...
        class Chunk final : public sf::Transformable, public Drawable {
            public:
                ...
                Chunk(const tmx::TileLayer& layer, std::vector<const tmx::Tileset*> tilesets,
                    ...
                    auto vertColour = sf::Color(200, 200, 200, layerOpacity);
                    ...
                    std::size_t xPos = position.x / tileSize.x;
                    std::size_t yPos = position.y / tileSize.y;
                        ...
                        for (auto y = yPos; y < yPos + chunkTileCount.y; ++y) {
                            for (auto x = xPos; x < xPos + chunkTileCount.x; ++x) {
                                if (idx < m_chunkTileIDs.size() && m_chunkTileIDs[idx].ID >= ca->m_firstGID
                                    ...
                                    if (registerAnimation && m_animTiles.find(m_chunkTileIDs[idx].ID) != m_animTiles.
                                        end()) {
                                        AnimationState as;
                                    ...
                                    sf::Vector2f tileOffset(static_cast<float>(x) * mapTileSize.x,
                                                            static_cast<float>(y) * mapTileSize.y + mapTileSize.y - ca->
                                                            tileSetSize.y);

                                    auto idIndex = m_chunkTileIDs[idx].ID - ca->m_firstGID;
                                    sf::Vector2f tileIndex(
                                        sf::Vector2i(idIndex % ca->tsTileCount.x, idIndex / ca->tsTileCount.x));
            ...
            private:
                class ChunkArray final : public Drawable {
                    public:
                ...
                // stores all tiles in this chunk for later manipulation
        ...
        Chunk::Ptr& getChunkAndTransform(std::int32_t x, std::int32_t y, sf::Vector2u& chunkRelative) {
            ...
            chunkRelative.x = ((x * m_mapTileSize.x) - chunkX * static_cast<std::uint32_t>(m_chunkSize.x)) /
                m_mapTileSize.x;
            chunkRelative.y = ((y * m_mapTileSize.y) - chunkY * static_cast<std::uint32_t>(m_chunkSize.y)) /
                m_mapTileSize.y;
        ...
        void updateVisibility(const sf::View& view) const {
            ...
            for (auto y = posY; y < posY2; ++y) {
                for (auto x = posX; x < posX2; ++x) {
                    std::size_t idx = y * static_cast<std::int32_t>(m_chunkCount.x) + x;
...
inline ObjectLayer::ObjectLayer(const tmx::Map& map, std::size_t layerID) {
...
inline sf::Shape* ObjectLayer::createShape(const tmx::Object& object) {
...
inline void ObjectLayer::draw(sf::RenderTarget& target, sf::RenderStates states) const {
    // Apply the transform
    states.transform *= getTransform();
